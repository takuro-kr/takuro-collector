...
현재 개발 버전은 AMB 전용 매물·사진 파서, 검증형 업데이트 다운로드, 설정형 관리회사 어댑터를 포함한 `0.4.4`입니다. 위 ZIP/EXE 해시는 수정 전 0.4.0 배포 기준선의 식별값입니다.

0.4.3 빌드 식별값과 실제 회귀 결과는 `docs/releases/0.4.3.md`에 기록했습니다. 0.4.4 결과는 `docs/releases/0.4.4.md`에 기록합니다.
